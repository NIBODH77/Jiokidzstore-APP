...
def create_order(
    ...
    success, message, data = service.create_order(
...
def get_orders(
    ...
    service = OrderService(db)
    return service.get_user_orders(current_user.id, status, page, page_size)
...
def cancel_order(
    ...
    service = OrderService(db)
    success, message = service.cancel_order(order_id, current_user.id, request.reason)
    if not success:
        raise HTTPException(status_code=400, detail=message)
